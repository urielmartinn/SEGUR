<?php
// Zerrendaketa: localhost:81/items
...
<body>
  <h2>Disken zerrendaketa</h2>
  <a href="/add_item">Diska gehitu</a>
  <ul>
  <?php while($row = $result->fetch_assoc()): ?>
    <li>
      <?=htmlspecialchars($row['title'])?> - <?=htmlspecialchars($row['artist'])?>
      [<a href="/show_item?item=<?=urlencode($row['id'])?>">Ikusi</a>]
      [<a href="/modify_item?item=<?=urlencode($row['id'])?>">Editatu</a>]
      [<a href="/delete_item?item=<?=urlencode($row['id'])?>">Ezabatu</a>]
    </li>
  <?php endwhile; ?>
  </ul>
</body>
